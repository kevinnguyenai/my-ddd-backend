...
const UsersController = {
  get router() {
    ...
    router.post('/one', inject('getOneUser'), this.one);
  ...
  one(req, res, next) {
    const { getOneUser, userSerializer } = req;

    const { SUCCESS, ERROR, NOT_FOUND} = getOneUser.outputs;
    ...
    getOneUser.execute(String(req.body.email), String(req.body.password))
  },
